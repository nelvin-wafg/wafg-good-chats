...
//   KIT_NOTIFY_TAG_ID    · (optional) numeric tag ID to apply to landing-page notify
//                          signups. find it in Kit → Subscribers → Tags → click the
//                          tag → the number in the URL (/tags/1234567). kit's v3 API
//                          requires numeric IDs, not tag names.
//
...
export async function addSubscriberToKit({ email, firstName, linkedinUrl, tagId, sourceNote }) {
  const apiKey = process.env.KIT_API_KEY;
  const sequenceId = process.env.KIT_SEQUENCE_ID;
  const formId = process.env.KIT_FORM_ID;
  const linkedinField = process.env.KIT_LINKEDIN_FIELD || 'linkedin';
  const sourceField = process.env.KIT_SOURCE_FIELD || 'source';
  ...
  // map structured info onto kit custom fields when we have it. kit silently
  // ignores fields that don't exist, so this is safe even before you create them.
  const fields = {};
  if (linkedinUrl) fields[linkedinField] = linkedinUrl;
  if (sourceNote) fields[sourceField] = sourceNote;
  const hasFields = Object.keys(fields).length > 0;

  // kit accepts a `tags` array of numeric tag IDs. used to mark "good-chats-lead"
  // signups so you can segment them from regular event opt-ins later.
  // tagId must be a number — kit's v3 API ignores string tag names.
  const tagIdNum = tagId ? Number(tagId) : null;
  const tags = tagIdNum && !Number.isNaN(tagIdNum) ? [tagIdNum] : undefined;
